...
for (const filepath of new Bun.Glob("@griffin/*/package.json").scanSync({ cwd: "./dist" })) {
  ...
  JSON.stringify(
    {
      ...
      scripts: {
        // best-effort: clears a stale global @griffin/cli whose `griffin`
...
for (const [name] of Object.entries(binaries)) {
  try {
    if (!name.includes("windows")) {
      await $`chmod 755 ./dist/${name}/bin/griffin`
    }
...
if (failed.length > 0) {
  ...
  throw new Error("Refusing to publish @griffin/griffin wrapper because one or more platform packages failed")
}
...
if (!Script.preview) {
  try {
    ...
    const arm64Sha = await $`sha256sum ./dist/griffin-linux-arm64.tar.gz | cut -d' ' -f1`
      .text()
      .then((x) => x.trim())
    const x64Sha = await $`sha256sum ./dist/griffin-linux-x64.tar.gz | cut -d' ' -f1`.text().then((x) => x.trim())
    const macX64Sha = await $`sha256sum ./dist/griffin-darwin-x64.zip | cut -d' ' -f1`.text().then((x) => x.trim())
    const macArm64Sha = await $`sha256sum ./dist/griffin-darwin-arm64.zip | cut -d' ' -f1`
      .text()
      .then((x) => x.trim())
    ...
    const homebrewFormula = [
      ...
      `      url "/releases/download/v${Script.version}/griffin-darwin-x64.zip"`,
      `      sha256 "${macX64Sha}"`,
      "",
      "      def install",
      '        bin.install "griffin"',
      ...
      "    end",
      "    if Hardware::CPU.arm?",
      `      url "/releases/download/v${Script.version}/griffin-darwin-arm64.zip"`,
      `      sha256 "${macArm64Sha}"`,
      "",
      "      def install",
      '        bin.install "griffin"',
      ...
      `      url "/releases/download/v${Script.version}/griffin-linux-x64.tar.gz"`,
      `      sha256 "${x64Sha}"`,
      "      def install",
      '        bin.install "griffin"',
      "      end",
      ...
      `      url "/releases/download/v${Script.version}/griffin-linux-arm64.tar.gz"`,
      `      sha256 "${arm64Sha}"`,
      "      def install",
      '        bin.install "griffin"',
      "      end",
    ...
    if (!token) {
      ...
      await Bun.file("./dist/homebrew-tap/griffin.rb").write(homebrewFormula)
      await $`cd ./dist/homebrew-tap && git add griffin.rb`
