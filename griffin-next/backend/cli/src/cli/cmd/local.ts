...
export async function runLocalModelSetup(input: LocalSetupInput = {}): Promise<string | null> {
  return Instance.provide({
    ...
    async fn() {
      ...
      const scopeLabel = input.project ? "this project's griffin.json" : "your global config"
      prompts.log.success(`Added ${selected.length} model(s) under provider "${id}" to ${scopeLabel}.`)

      // Offer to set a default so `griffin run` uses it immediately.
      ...
      prompts.log.message(`Use it now:  griffin run --model ${firstModel} "hello"`)
      return id
  ...
  handler: async () => {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (!locals.length) {
          UI.println("No local model providers configured. Add one with `griffin local add`.")
          return
        }
...
export const LocalCommand = cmd({
  ...
  handler: async () => {
    // Bare `griffin local` runs the add wizard (AddCommand is the $0 default,
