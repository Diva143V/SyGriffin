...
async function openInEditor(initial: string): Promise<string> {
  ...
  const file = path.join(os.tmpdir(), `griffin-skill-${Date.now()}.md`)
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (!info || !(info.location ?? "").includes("/user-skills/")) {
          UI.error(`No user skill named "${name}". Create one with: griffin skill new ${name}`)
          process.exit(1)
          return
        }
...
const SkillListCommand = cmd({
  ...
  builder: (yargs: Argv) =>
    yargs.option("all", {
      describe: "include bundled Griffin skills in the listing",
      type: "boolean",
      default: false,
    }),
  handler: async (args) => {
    // Skill.state() needs project-instance context so it can walk the
    // .claude/.griffin config dirs alongside the global cache + learned +
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        UI.println(`Griffin bundled skills: ${totalBundled}`)
        ...
        if (totalBundled === 0) {
          UI.println(
            "Bundled Griffin skills did not load. Run `griffin login` and `griffin sync`, then try again.",
          )
          UI.println("Offline fallback only shows skills already cached on this machine.")
          UI.println("")
        } else if (totalBundled < 100) {
          UI.println(
            "Only a small bundled skill set is visible; this usually means the full Griffin skill index was not fetched.",
          )
          UI.println("Run `griffin sync` or try again once online/authenticated.")
          UI.println("")
        } else if (!showAll) {
          UI.println("Use `griffin skill list --all` to show bundled Griffin skills.")
          UI.println("")
        }
        ...
        if (totalLearned === 0 && totalInstalled === 0) {
          UI.println("Install third-party skills with: griffin skill add <git-url>")
          return
        }
        ...
        if (totalInstalled > 0) UI.println(`Run \`griffin skill show <namespace>[/<name>]\` for details.`)
      },
    })
  },
})
  ...
  handler: async (args) => {
    ...
    if (target.includes("/")) {
      ...
      try {
        ...
        UI.println("")
        UI.println(`(SKILL.md not on disk at ${skillPath} — run \`griffin\` to trigger sync)`)
      }
      return
    }
...
const SkillSetEntriesCommand = cmd({
  ...
  describe:
    ...
    "Writes griffin-skills.json locally for the namespace.",
  builder: (yargs: Argv) => {
    return yargs
