...
export namespace LSPServer {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Bun.file(js).exists())) {
          if (Flag.GRIFFIN_DISABLE_LSP_DOWNLOAD) return
  ...
  export const Ty: Info = {
    ...
    async spawn(root) {
      if (!Flag.GRIFFIN_EXPERIMENTAL_LSP_TY) {
        return undefined
      }
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Bun.file(binary).exists())) {
          ...
          const response = await fetch("/archive/refs/heads/master.zip")
          if (!response.ok) return
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "griffin-jdtls-data"))
      return {
        process: spawn(
          java,
          [
