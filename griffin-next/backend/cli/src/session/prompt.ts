...
export namespace SessionPrompt {
  ...
  export const OUTPUT_TOKEN_MAX = Flag.GRIFFIN_OUTPUT_TOKEN_MAX || 2_000
  ...
  export const loop = fn(Identifier.schema("session"), async (sessionID) => {
    ...
    while (true) {
      ...
      if (!model) {
        const error = new NamedError.Unknown({
          message: `Model ${lastUser.model.providerID}/${lastUser.model.modelID} is not available. Connect a provider in Griffin Settings, then choose a model.`,
        }).toObject()
        Bus.publish(Session.Event.Error, { sessionID, error })
        await Session.updateMessage({
          id: await MessageV2.nextMessageID(sessionID),
      ...
      if (step === 1 && memoryQuery) void Memory.remember(memoryQuery, sessionID)
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    // Add compute routing only when the user configured an explicit preference.
    if (COMPUTE_AGENTS.has(input.agent.name) && (await Config.get()).billing?.compute) {
      ...
      userMessage.parts.push({
        ...
        type: "text",
        text: managed
          ? "<system-reminder>Compute spend is set to managed. Confirm expected cost before launching any paid compute job.</system-reminder>"
          : "<system-reminder>Run compute on the user's connected providers and confirm expected cost before launching any paid job.</system-reminder>",
        synthetic: true,
      })
    }
