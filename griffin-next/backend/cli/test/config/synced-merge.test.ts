...
// The Atlas sync writes griffin-synced.json into the user's XDG config dir; it's
// read fresh per-instance by Config.state(). The user's own config goes in the
// project's griffin.json (via tmpdir({config})), which is per-test isolated —
// unlike the global griffin.json, which Config caches process-wide. These tests
...
describe("Atlas synced-config merge", () => {
  ...
  test("a custom BYOK provider in griffin.json is untouched by sync (#142)", async () => {
    await writeSynced({
