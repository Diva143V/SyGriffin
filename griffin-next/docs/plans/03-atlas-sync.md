...
Workstream: make sync between Griffin and Atlas correct and resilient — identity, entitlements, and state stay consistent; sync recovers from interruption; no silent divergence. Griffin paths under `backend/cli/src/`; Atlas under `backend/app/` (cloned reference).
...
**On disk:** `synced-env.json` (flat env, replayed synchronously at boot by `preload-env.ts:28-51`, **only where the var is unset** — "shell exports win"); `griffin-synced.json` (model lockdown, highest-priority config layer, `config.ts:190-193`); `griffin-session.json` (`api_key`, `user_id`, `cached_v`, `last_check_ts`); the bundled atlas-cli config (re-seeded on every sync). Sign-out (`clearSession`, `index.ts:501-520`) unlinks all of these + drops the usage queue. Server 401 → auto-clear; 403/402 → keep session, return `null`.
...
- **G3 (HIGH) — non-atomic writes brick config.** `synced-env.json` and `griffin-synced.json` are plain `Bun.write` (no temp+rename), 4 independent writes, no barrier. A torn `synced-env.json` → `preload-env` swallows it → managed keys silently vanish. A torn `griffin-synced.json` → `config.ts:193` load **throws** uncaught in `Config.state()` → **CLI unusable until the file is removed by hand**.
