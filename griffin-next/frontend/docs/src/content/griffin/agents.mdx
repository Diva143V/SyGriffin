...
Run `griffin agent list` to print the complete set on your install, including utility sub-agents like `explore` and `reviewer`.

## Pick an agent

```bash
# one-shot run with a specific agent
griffin run --agent physics "Fit the dispersion relation in data/spectra.csv"
```

In the workspace, switch agents from the session picker. To change the default, set `default_agent` in `griffin.json`. Only agents with mode `primary` or `all` can lead a session; `subagent` profiles are reachable only by delegation.

## Create a custom agent

`griffin agent create` walks you through it interactively. The non-interactive form:

```bash
griffin agent create \
  --path .griffin \
...
`--tools "read,grep,glob"` produces a read-only agent that cannot be tricked into editing files. The available tools are `bash`, `read`, `write`, `edit`, `list`, `glob`, `grep`, `webfetch`, `task`, `todowrite`, and `todoread` — run `griffin agent create --help` for the list on your version.

## Where agents live

```text
.griffin/agent/             # project-scoped
~/.config/griffin/agent/    # user-global
<bundled>                       # ships with Griffin
```
...
<Columns cols={2}>
  ...
  <Card title="Sessions" icon="terminal" href="/griffin/sessions">
