...
Griffin is open source under Apache-2.0, so the whole security model below is auditable in the [repository](https://github.com/synthetic-sciences/griffin). The agent runs locally with the same filesystem and shell access you have. Two principles anchor the design: keep credentials out of subprocesses that do not need them, and make the trust boundary explicit instead of pretending the agent is a sandbox.
...
You can add a real boundary: the opt-in [execution sandbox](/griffin/sandbox) wraps the agent's shell commands in an OS sandbox (macOS Seatbelt, Linux bubblewrap) that confines writes to the workspace and can deny network egress. Turn it on with `griffin sandbox enable` and verify it with `griffin sandbox test`. It is write-containment, not a full jail — for hostile code, still run Griffin inside a container or a VM.
...
| Atlas session | `~/.config/griffin/config.json` | `thk_*` key created by `griffin login`; revocable from the dashboard. |
| Synced service credentials | `~/.config/griffin/credentials.json` | Present only when [connected to Atlas](/griffin/atlas); refresh with `griffin sync`. |
| Native binary (curl install) | `~/.griffin/bin/griffin` | Added to PATH through your shell rc. |

Override the config parent directory with `XDG_CONFIG_HOME`. Run `griffin debug paths` to print the resolved data, config, cache, and state directories on your machine.
...
- Nothing else, in BYOK mode. If you [connect to Atlas](/griffin/atlas), synced credentials and usage metering are held against your account.
...
Report security issues through the GitHub Security Advisory ["Report a Vulnerability"](https://github.com/synthetic-sciences/griffin/security/advisories/new) form — not on public issue trackers. If you do not hear back within six business days, email security@syntheticsciences.ai.
