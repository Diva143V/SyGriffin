...
async function highlightCodeBlocks(html: string): Promise<string> {
  ...
  const [highlighter, bundledLanguages] = await Promise.all([
    getSharedHighlighter({ themes: ["Griffin"], langs: [] }),
    loadLangs(),
  ])
...
let jsParserP: Promise<ReturnType<typeof marked.use>> | undefined
function loadJsParser() {
  return (jsParserP ??= (async () => {
    const [{ default: markedKatex }, { default: markedShiki }, bundledLanguages] = await Promise.all([
    ...
    return marked.use(
      ...
      markedShiki({
        async highlight(code, lang) {
          const highlighter = await getSharedHighlighter({ themes: ["Griffin"], langs: [] })
          ...
          return highlighter.codeToHtml(code, { lang: lang || "text", theme: "Griffin", tabindex: false })
        },
      }),
    )
  })())
