...
// Basic-Auth creds for the in-process griffin server. e2e-local.ts pins both
// GRIFFIN_SERVER_* (server side) and VITE_GRIFFIN_SERVER_* (frontend side) to
// the same value so the Playwright-hosted frontend can authenticate.
const serverUsername = process.env.VITE_GRIFFIN_SERVER_USERNAME ?? "griffin"
const serverPassword = process.env.VITE_GRIFFIN_SERVER_PASSWORD ?? ""
...
export default defineConfig({
  ...
  webServer: {
    ...
    timeout: 120_000,
    env: {
      VITE_GRIFFIN_SERVER_HOST: serverHost,
      VITE_GRIFFIN_SERVER_PORT: serverPort,
      VITE_GRIFFIN_SERVER_USERNAME: serverUsername,
      VITE_GRIFFIN_SERVER_PASSWORD: serverPassword,
    },
  },
  use: {
    baseURL,
    ...
    // griffin-fetch.ts wraps fetch() with the same header, but its
