  ...
  const [root] = createResource(
    ...
    async ([dir]) => {
      ...
      try {
        ...
        if (status === 403) {
          const message = err?.body?.message ?? err?.message ?? "Griffin cannot read this directory"
                ...
                >
                  ...
                  >
                    <Show when={isMac()} fallback={<>{permissionError()}</>}>
                      Grant <strong>Full Disk Access</strong> to Griffin in System Settings → Privacy &amp;
  ...
  return (
    <Show when={matches() || isOpen()}>
      <button
        ...
        title={props.node.ignored ? `${props.node.name} · ignored by gitignore / Griffin` : props.node.name}
        style={{
          all: "unset",
          cursor: "pointer",
          display: "flex",
