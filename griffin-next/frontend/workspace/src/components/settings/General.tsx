...
export default function General() {
  ...
  const signOut = async () => {
    if (!window.confirm("Disconnect this local server from Griffin?")) return
    setBusy(true)
    try {
  ...
  return (
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-8">
      ...
      <div class="flex flex-col gap-8 w-full max-w-[760px]">
        ...
        {/* Account */}
        <Section title="Account" description="Your Griffin identity and subscription.">
          <div class="border border-border-weak-base rounded-[4px] overflow-hidden bg-surface-base/40">
            ...
            <Row title="Session" description="Disconnect this machine from Griffin.">
              <Button
                size="small"
                variant="secondary"
            ...
            <Show when={account()?.session === false}>
              <div class="px-4 py-3">
                <p class="text-12-regular text-text-weak">
                  Signed out — run{" "}
                  <code style={{ "font-family": FONT_CODE, "font-size": "11px" }}>griffin connect login</code> in a
        ...
        {/* Licensing */}
        <Section title="Licensing" description="How you intend to use outputs from Griffin.">
