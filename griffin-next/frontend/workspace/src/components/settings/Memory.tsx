...
type MemoryStatus = { provider: "supermemory"; mode: "local" | "cloud"; connected: boolean; baseURL: string }
type Doc = { enabled: boolean; categories: Category[]; status?: MemoryStatus }
      ...
      <div class="sticky top-0 z-10 bg-[linear-gradient(to_bottom,var(--surface-raised-stronger-non-alpha)_calc(100%_-_24px),transparent)]">
        <div class="flex flex-col gap-1 px-4 py-8 sm:p-8 max-w-[760px]">
          ...
          <p class="text-13-regular text-text-weak">
            Supermemory keeps durable context across Griffin sessions. Pin important preferences or instructions here.
          </p>
        </div>
      </div>
          ...
          {(status) => (
            <div class="flex items-center justify-between gap-3 rounded-[4px] border border-border-weak-base bg-surface-base/40 px-4 py-3">
              <div class="flex flex-col gap-0.5 min-w-0">
                <span class="text-13-medium text-text-strong">Supermemory</span>
                <span class="text-12-regular text-text-weak truncate">
                  {status().mode === "local" ? "Local context service" : "Cloud context service"} · {status().baseURL}
                </span>
              </div>
              <span
                class="text-12-medium"
                classList={{ "text-text-success": status().connected, "text-text-warning": !status().connected }}
              >
        ...
        <div class="flex items-center justify-between gap-3 rounded-[4px] border border-border-weak-base bg-surface-base/40 px-4 py-3">
          <div class="flex flex-col gap-0.5 min-w-0">
            ...
            <span class="text-12-regular text-text-weak">
              {doc().enabled ? "Project context and pinned notes are recalled." : "Recall is paused; pinned notes stay saved."}
            </span>
          </div>
