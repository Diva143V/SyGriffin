// Storage — real on-disk footprint of the Griffin data directory, a
...
export const Storage: Component = () => {
  ...
  const relocate = async () => {
    ...
    try {
      ...
      setStatus(`Data copied to ${res.target}. Restart Griffin to use the new location.`)
      await load()
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    } finally {
      ...
      <div class="sticky top-0 z-10 bg-[linear-gradient(to_bottom,var(--surface-raised-stronger-non-alpha)_calc(100%_-_24px),transparent)]">
        <div class="flex flex-col gap-1 px-4 py-8 sm:p-8 max-w-[760px]">
          ...
          <p class="text-13-regular text-text-weak">
            Where Griffin keeps data on disk, and how much space it uses.
          </p>
        </div>
      </div>
